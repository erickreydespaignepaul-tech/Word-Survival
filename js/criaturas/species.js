// Lista generada de 100 especies (metadata en español) con biomas preferidos y estilo de patas
export const ESPECIES = (() => {
  ...
  const biomasDisponibles = ['bosque', 'sabana', 'tundra', 'desierto', 'jungla', 'pantano', 'montana', 'playa', 'pradera', 'taiga'];
  const especies = [];
  for (let i = 0; i < 100; i++) {
    const base = nombresBase[i % nombresBase.length];
    const nombre = `${base}-${i}`;
    const biomaPrincipal = biomasDisponibles[i % biomasDisponibles.length];
    // asignar 1-2 biomas preferidos
    const biomasPref = [biomaPrincipal];
    if (i % 7 === 0) biomasPref.push(biomasDisponibles[(i + 3) % biomasDisponibles.length]);

    // recomendar estilo de patas por base
    let legStyle = 'rect';
    if (base === 'pollo' || base === 'aguila') legStyle = 'triangulo';
    if (base === 'oveja' || base === 'vaca' || base === 'cerdo') legStyle = 'rect';
    if (base === 'humano') legStyle = 'triangulo';
    if (base === 'araña') legStyle = 'linea';

    especies.push({
      id: i,
      nombre,
      base,
      biomasPreferidos: biomasPref,
