...
function seededNoise(seed, x, y) {
  // mezcla bitwise para obtener pseudoaleatorio determinista por (x,y,seed)
...
export class Chunks {
  ...
  biome(wx, wy) {
    // usa mapas de temperatura y humedad para decidir bioma
  ...
  generar(cx, cy) {
    ...
    for (let x = 0; x < this.tamano; x++) {
      for (let y = 0; y < this.tamano; y++) {
        ...
        // reglas por bioma
        if (b === 'desierto') { if (tipo === BLOQUES.AGUA || tipo === BLOQUES.ARBOL) tipo = BLOQUES.TIERRA; if (n < 0.03) tipo = BLOQUES.ROCA; }
        else if (b === 'nieve') { if (tipo === BLOQUES.AGUA) tipo = BLOQUES.HIERBA; if (n < 0.12) tipo = BLOQUES.ROCA; }
        else if (b === 'montana') { tipo = montaña ? BLOQUES.ROCA : (tipo === BLOQUES.AGUA ? BLOQUES.TIERRA : tipo); }
        else if (b === 'pantano') { if (n < 0.13) tipo = BLOQUES.AGUA; else if (tipo === BLOQUES.ARBOL) tipo = BLOQUES.HIERBA; }
        else if (b === 'jungla') { if (n > 0.8) tipo = BLOQUES.ARBOL; if (n < 0.02) tipo = BLOQUES.ROCA; }
        else if (b === 'rio') { tipo = BLOQUES.AGUA; }
